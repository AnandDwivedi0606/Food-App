...
import OnlinePickUpDeals from './OnlinePickUpDeals'
import OnlinePickUpList from './OnlinePickUpList'
import OnlineRecommanded from './OnlineRecommanded'
import EmptyCard from '../../EmptyCard'
import { icon_image_id } from '../../../../Constants/constants'

export default function OnlineDeliveryDetails() {
  ...
  async function getRestaurantsInfo() {
    ...
    setTopPick(response?.data?.data?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[1]?.card?.card?.carousel);
    setTopPickTitle(response?.data?.data?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[1]?.card?.card?.title);
    ...
    setTopRecommandedTitle(response?.data?.data?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[2]?.card?.card?.title);
    setTopRecommanded(response?.data?.data?.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards[2]?.card?.card?.itemCards);
  ...
  return (restaurantsInfo.length === 0) ? <EmptyCard /> : (
    <>
      <div className="container">
        ...
        <OnlinePickUpDeals deals={deals} />

        <hr />

        <OnlinePickUpList topPicktitle={topPicktitle} topPick={topPick} />
        <hr />
        <OnlineRecommanded toprecommandedtitle={toprecommandedtitle} toprecommanded={toprecommanded} />
        <hr />
      </div>
    </>
  )
