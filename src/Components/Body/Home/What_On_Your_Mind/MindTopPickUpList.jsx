...
export default function MindTopPickUpList(props) {
  return (
    <>
      <div className="container w-75 mx-auto mt-5">
        <h3>{props?.mindtopPicktitle} ({(props?.mindtopPick)?.length ? (props?.mindtopPick)?.length : 0})</h3>
        <div className="row horizontal_view_top my-5">
          {
            props?.mindtopPick?.map((item) => {
